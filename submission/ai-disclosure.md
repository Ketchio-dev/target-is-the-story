...
- **The check and sabotage suites.** `check.py` (28 checks) and `sabotage.py` (23 planted
...
- **The Inspiration section** is written by the author. The model was instructed not to invent
  personal experience and did not.
- **No number was produced by a model.** Every figure on the page and in the write-up comes
  from Tribunals Ontario's published documents, through the parsers, and is re-verified against
  `analyze.py`'s output by a check. The model never typed a result into the text; when a number
  in the README drifted from the code, a check caught it.
- **Submission, repository publication, and the video upload** are the author's actions.
...
1. **A fixed-denominator check suite.** 28 checks whose count cannot shrink when one fails.
   A check that silently disappears is worse than a check that fails.
2. **Sabotage testing.** 23 deliberate defects are injected into the source one at a time, and
   ...
   "the mutation changed no behaviour" need different fixes. Current: 23 of 23 caught.
